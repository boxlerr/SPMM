...
| **Traer historial** (por producto cód+desc) | 🟡 Código listo, pendiente deploy | Endpoint `GET /ordenes/historial-procesos?id_articulo=` (última OT del mismo producto) + botón activo en el modal. Query real sin probar (no hay DB acá). |
...
- `components/CreateWorkOrderModal.tsx` — botón **"Traer historial"**: `handleTraerHistorial` llama al endpoint por `id_articulo` y carga los procesos en el listado (todos tildados).

### Backend — Traer historial
- `infrastructure/OrdenTrabajoRepository.py` — `obtener_historial_procesos(id_articulo, excluir_orden_id)`: procesos de la última OT del mismo producto.
- `application/OrdenTrabajoService.py` — `obtenerHistorialProcesos(...)` serializa al shape del front.
- `presentation/OrdenTrabajoAPI.py` — `GET /ordenes/historial-procesos` (antes de `/ordenes/{id}`).
- Tests: `backend/tests/test_historial_procesos.py` (serialización + vacío).
