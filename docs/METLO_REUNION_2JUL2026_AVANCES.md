...
|---|---|---|
| Rehacer carga como **listado** (proceso·min·orden·máquina·cant.emp.) | 🟡 Código listo (modal) | Componente `ProcesosEditor` integrado en `CreateWorkOrderModal`. Falta replicar en alta inline (`AddProcessRow`) + deploy. |
...
- `AddProcessRow` (alta inline) al mismo diseño + `id_maquinaria` (la "visual de agregar" que faltaba).
