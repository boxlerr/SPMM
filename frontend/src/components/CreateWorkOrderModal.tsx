...
export default function CreateWorkOrderModal({ isOpen, onClose, onSuccess }: CreateWorkOrderModalProps) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [prioridadesRes, procesosRes, operariosRes, maquinariasRes, articulosRes] = await Promise.all([
    ...
    const validateForm = () => {
        if (!generalData.cliente || !generalData.descripcion || !generalData.prioridad_id || !generalData.articulo_id) {
    ...
    const performSubmission = async () => {
        ...
        const fullData = {
            ...
            observaciones: generalData.descripcion,
    ...
    const handleAttemptClose = () => {
        ...
        const isDirty = generalData.cliente || generalData.descripcion || processes.length > 0;
    ...
    return (
        <>
            ...
            <Dialog open={isOpen} onOpenChange={(open) => !open && handleAttemptClose()}>
                ...
                <DialogContent className="sm:max-w-[95vw] md:max-w-[1200px] bg-white rounded-xl shadow-2xl border-0 max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden">
