...
export function DeliveryProgress({ total, delivered, compact = false, className }: DeliveryProgressProps) {
    ...
    return (
        <div className={cn("p-3 bg-white rounded-xl border border-gray-100 shadow-sm", className)}>
            <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                    <h4 className="text-xs font-bold text-gray-900 uppercase tracking-widest flex items-center gap-2">
                        Entrega
                    </h4>
                    <Badge variant={badgeVariant} className={cn("px-2 py-0 h-5 text-[10px]", badgeClass)}>
                        {icon} {statusText}
                    </Badge>
                </div>
                ...
                {isOverflow && (
                    <div className="mt-1 text-[10px] text-orange-600 font-medium flex items-center bg-orange-50 p-1.5 rounded-md">
                        <AlertTriangle className="w-3 h-3 mr-1.5" />
                        Excede total planificado.
                    </div>
                )}
            </div>
        </div>
