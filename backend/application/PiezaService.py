...
class PiezaService:
    ...
    async def listarPiezas(self):
        logger.info("Service - Listar piezas.")
        piezas = await self.repository.find_all()
        return ResponseDTO(status=True, data=jsonable_encoder(piezas))
